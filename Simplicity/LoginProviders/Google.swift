...
public class Google: OAuth2 {
    ...
    private func exchangeCodeForAccessToken(_ authorizationCode: String, callback: @escaping ExternalLoginCallback) {
        ...
        let task = session.dataTask(with: request) { (data, response, error) -> Void in
            guard let data = data, let json = (try? JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any], let accessToken = json["access_token"] as? String, let idToken = json["id_token"] as? String else {
            ...
            self.loadProfile(accessToken: accessToken, handler: { profile in
                callback(accessToken, idToken, profile, nil)
            })
        }
        task.resume()
    }
    ...
    private func loadProfile(accessToken: String, handler: @escaping ([String: Any]?) -> Void) {
        let session = URLSession(configuration: URLSessionConfiguration.ephemeral)
        let url = URL(string: "https://www.googleapis.com/plus/v1/people/me?access_token=" + accessToken)!
